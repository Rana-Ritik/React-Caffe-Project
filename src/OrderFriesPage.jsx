...
const OrderFriesPage = () => {
  const location = useLocation();
  const navigate = useNavigate();

  // Get the selected fries from the location state
  const { selectedFries } = location.state || {};
  const { user } = useContext(UserContext);

  if (!selectedFries) {
    return <div>No fries selected</div>;
  }
  ...
  const handleSubmitOrder = (e) => {
    ...
    const orderData = {
      ...
      productname: selectedFries.name,
      productprice: parseFloat(selectedFries.price), // Ensure price is a number
      productdescription: selectedFries.description,
      productimage: selectedFries.image, // Adding image URL if needed
    };
    ...
    try {
      ...
      .then((res) => {
        ...
        alert(`Your coffee order has been placed successfully! for ${selectedFries.name}`);
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto px-4 py-12">
        <h2 className="text-4xl font-bold text-center mb-8">Order Your Fries</h2>

        {/* Fries Details Section */}
        <div className="flex flex-col md:flex-row bg-white p-6 rounded-lg shadow-lg mb-8">
          <div className="md:w-1/2">
            <img
              src={selectedFries.image}
              alt={selectedFries.name}
              className="w-full h-full object-cover rounded-lg"
              style={{ maxHeight: '300px', objectPosition: 'center' }} // Ensures image is centered and contained
            />
          </div>
          <div className="md:w-1/2 p-6">
            <h3 className="text-3xl font-semibold text-gray-800">{selectedFries.name}</h3>
            <p className="text-xl text-gray-600">{selectedFries.price}</p>
            <p className="text-lg text-gray-600 mt-4">{selectedFries.description}</p>
          </div>
        </div>
