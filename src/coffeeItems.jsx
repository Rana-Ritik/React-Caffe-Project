...
// Coffee items data (same as in CoffeePage)
const coffeeItems = [
  {
    id: 1,
    category:'coffee',
...
const CoffeeItemPage = () => {
  const { coffeeId } = useParams(); // Get the coffee item ID from the URL
  const navigate = useNavigate();
  // Find the coffee item by id
  const coffee = coffeeItems.find(item => item.id === parseInt(coffeeId));
  ...
  const handleOrderClick = () => {
    ...
    navigate('/ordercoffee', { state: { selectedCoffee: coffee } });
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto p-8 text-center">
        <h2 className="text-4xl font-bold mb-6">{coffee.name}</h2>
        ...
        <p className="text-xl font-semibold text-gray-800 mb-6">{coffee.price}</p>

        <button  onClick={handleOrderClick} className="bg-green-500 text-white py-2 px-6 rounded-lg hover:bg-green-700 transition-colors duration-300">
          Order Now
        </button>
      </div>
