...
const MyOrders = () => {
  ...
  useEffect(() => {
   ...
   if(token){
    axios.get("http://localhost:5000/order",{headers:{"Token":token}})
          ...
          <div>
            {orders.map((order, index) => (
              <div key={index} className="bg-white rounded-lg shadow-lg p-6 mb-6">
                ...
                {order.productCategory === 'pizza' ? (
                  <div className="mt-4 text-gray-700">
                    <DetailRow label="Storage" value={order.productdescription} />
                    
                  </div>
                ) : order.productCategory === 'burger' ? (
                  <div className="mt-4 text-gray-700">
                    <DetailRow label="Speciality" value={order.productdescription} />
                ...
                ) : order.productCategory === 'fries' ? (
                    <div className="mt-4 text-gray-700">
                      <DetailRow label="Speciality" value={order.productdescription} />
                      
                    </div>
                    ): order.productCategory === 'momo' ? (
                    <div className="mt-4 text-gray-700">
                      <DetailRow label="Speciality" value={order.productdescription} />
                      
                    </div>
                    ): order.productCategory === 'pasta' ? (
                    <div className="mt-4 text-gray-700">
                      <DetailRow label="Speciality" value={order.productdescription} />
                      
                    </div>):
                     order.productCategory === 'shake' ? (
                        <div className="mt-4 text-gray-700">
                          <DetailRow label="Speciality" value={order.productdescription} />
                          
                        </div>): order.productCategory === 'smoothie' ? (
                    <div className="mt-4 text-gray-700">
                      <DetailRow label="Speciality" value={order.productdescription} />
