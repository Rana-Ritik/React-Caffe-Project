...
const OrderCoffeePage = () => {
  const location = useLocation();
  const navigate = useNavigate();

  // Get the selected coffee from the location state
  const { selectedCoffee } = location.state || {};
  const { user } = useContext(UserContext);

  if (!selectedCoffee) {
    return <div>No coffee selected</div>;
  }
  ...
  const [customerDetails, setCustomerDetails] = useState({
    ...
    phone: '',
    address: '',
    email: '', // Adding email field for consistency with other pages
  });
  ...
  const handleSubmitOrder = (e) => {
    ...
    const orderData = {
      ...
      productname: selectedCoffee.name,
      productprice: parseFloat(selectedCoffee.price), // Ensure price is a number
      productdescription: selectedCoffee.description,
      productimage: selectedCoffee.image, // Adding image URL if needed
    };
    ...
    try {
      ...
      .then((res) => {
        ...
        alert(`Your coffee order has been placed successfully! for ${selectedCoffee.name}`);
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto px-4 py-12">
        <h2 className="text-4xl font-bold text-center mb-8">Order Your Coffee</h2>
        ...
        <div className="flex flex-col md:flex-row bg-white p-6 rounded-lg shadow-lg mb-8">
          <div className="md:w-1/2">
            <img
              ...
              className="w-full h-full object-cover object-center rounded-lg max-h-72"
              style={{ objectFit: 'cover' }} // Ensures image fits well
            />
          </div>
          <div className="md:w-1/2 p-6">
            <h3 className="text-3xl font-semibold text-gray-800">{selectedCoffee.name}</h3>
            <p className="text-xl text-gray-600">{selectedCoffee.tprice}</p>
            <p className="text-lg text-gray-600 mt-4">{selectedCoffee.description}</p>
          </div>
        </div>
...
export default OrderCoffeePage;
