...
const OrderMomoPage = () => {
  ...
  // Get the selected momo from the location state
  const { selectedMomo } = location.state || {};
  const { user } = useContext(UserContext);

  if (!selectedMomo) {
    return <div>No momo selected</div>;
  }
  ...
  const handleSubmitOrder = (e) => {
    ...
    const orderData = {
      ...
      productCategory: 'momo',
      productname: selectedMomo.name,
      productprice: parseFloat(selectedMomo.price), // Ensure price is a number
      productdescription: selectedMomo.description,
      productimage: selectedMomo.image, // Adding image URL if needed
    };
    ...
    try {
      ...
      .then((res) => {
        ...
        alert(`Your coffee order has been placed successfully! for ${selectedMomo.name}`);
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto px-4 py-12">
        <h2 className="text-4xl font-bold text-center mb-8">Order Your Momo</h2>
        ...
        <div className="flex flex-col md:flex-row bg-white p-6 rounded-lg shadow-lg mb-8">
          ...
          <div className="md:w-1/2 p-6">
            <h3 className="text-3xl font-semibold text-gray-800">{selectedMomo.name}</h3>
            <p className="text-xl text-gray-600">{selectedMomo.price}</p>
            <p className="text-lg text-gray-600 mt-4">{selectedMomo.description}</p>
          </div>
        </div>
