...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await axios.get('http://localhost:5000/order/allorders', {
          headers: { token },
        });
    ...
    <div className="min-h-screen bg-gradient-to-r from-blue-400 via-blue-500 to-indigo-600 p-8">
      <div className="container mx-auto">
        ...
        <button
          ...
          className="absolute top-6 right-6 bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 focus:outline-none transition duration-300"
        >
          Logout
        </button>
              ...
              <div className="mt-6">
                {feedbacks.map((feedback, index) => (
                  <div key={index} className="bg-gray-100 p-4 rounded-md shadow-md hover:bg-yellow-50 transition duration-300 ease-in-out mb-4">
                    ...
                    <p className="text-gray-600"><strong>Feedback:</strong> {feedback.feedback}</p>
                  </div>
                ))}
              </div>
            ) : (
